...
const fetch_price_1 = require("../tools/fetch_price");
...
class SolanaResolveDomainTool extends tools_1.Tool {
    constructor(solanaKit) {
        ...
        this.description = `Resolve a .sol domain to a Solana PublicKey.
...
class SolanaFetchPriceTool extends tools_1.Tool {
  ...
  - tokenId: string, the mint address of the token, e.g., "JUPyiwrYJFskUPiHa7hkeR8VUtAeFoSYbKedZNsDvCN"`;
    ...
    async _call(input) {
        try {
            const price = await (0, fetch_price_1.fetchPrice)(this.solanaKit, input.trim());
            return JSON.stringify({
                status: "success",
                tokenId: input.trim(),
                priceInUSDC: price,
