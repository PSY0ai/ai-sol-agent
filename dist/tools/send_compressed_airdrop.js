...
async function sendCompressedAirdrop(agent, mintAddress, amount, decimals, recipients, priorityFeeInLamports, shouldLog = false) {
    ...
    try {
        sourceTokenAccount = await (0, spl_token_1.getOrCreateAssociatedTokenAccount)(agent.connection, agent.wallet, mintAddress, agent.wallet.publicKey);
    }
    catch (error) {
...
async function processAll(agent, amount, mint, recipients, priorityFeeInLamports, shouldLog) {
    ...
    for (let i = 0; i < instructionSets.length; i += MAX_CONCURRENT_TXS) {
        const batchPromises = instructionSets
            ...
            .map((instructions, idx) => sendTransactionWithRetry(rpc, instructions, payer, lookupTableAccount, i + idx).then((signature) => {
            confirmedCount++;
    ...
    if (failures.length > 0) {
        throw new Error(`Failed to process ${failures.length} batches: ${failures
            .map((f) => f.error)
            .join(", ")}`);
    }
...
async function sendTransactionWithRetry(connection, instructions, payer, lookupTableAccount, batchIndex) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
        try {
            ...
            const tx = (0, stateless_js_1.buildAndSignTx)(instructions, payer, blockhash, [], [lookupTableAccount]);
            const signature = await (0, stateless_js_1.sendAndConfirmTx)(connection, tx);
            return signature;
        }
        catch (error) {
            const isRetryable = error.message?.includes("blockhash not found") ||
